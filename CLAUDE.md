...
技术栈：Spring Boot 3.x + MyBatis-Plus + PostgreSQL 18（含 pgvector）+ Redis 7.x
...
- 对话消息持久化走 PostgreSQL，不缓存
- MCP Server 配置数量少，不缓存

### 部署架构（当前 50 人规模）
Nginx（静态文件 + API 反向代理）→ Spring Boot → PostgreSQL（含 pgvector）/ Redis / LLM API
...
- 主键：统一用 id，`bigint generated by default as identity`
- 时间字段：created_at、updated_at，`timestamp` 类型
- 逻辑删除：deleted，`smallint`，0=正常 1=删除
- 索引命名：idx_{表名}_{字段名}，PostgreSQL 里写成独立语句 `CREATE INDEX`
- 所有外键在应用层维护，不建数据库级外键约束
- 字符集：UTF-8（PostgreSQL 默认，建表无需指定）
- 大表预判：chat_message 增长最快，提前考虑分页查询性能
- 分页查询避免 SELECT COUNT(*)，用游标分页或估算总数

### 类型约定（PostgreSQL 严格类型，不做隐式转换）

MySQL 靠隐式转换兜住类型不匹配，PostgreSQL 会直接报错——以下几条是实际踩过的：

| 用途 | 用 | 不要用 | 原因 |
|---|---|---|---|
| 布尔 | `boolean` | `smallint` | Java 实体是 `Boolean`，smallint 列会报 `operator does not exist: smallint = boolean` |
| JSON | `jsonb` | `json` | 可索引、可用 `->>` 查询；写入依赖 JDBC 参数 `stringtype=unspecified` |
| 长文本 | `text` | `varchar(n)` | 无长度上限，不必猜一个够大的 n |
| 时间 | `timestamp` | `datetime` | PG 没有 datetime 类型 |
| 浮点 | `double precision` | `double` | PG 没有 double |
| 逻辑删除 | `smallint` | `boolean` | MyBatis-Plus 的 logic-delete-value 配的是 0/1 整数 |

**迁移 / seed 脚本显式插入 id 后必须重置序列**，否则下次自增会与已有 id 冲突：
